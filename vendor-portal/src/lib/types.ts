...
export interface PlanOffering {
  ...
  // Sourced from the real subscriptionPlans/{planId} docs (seedSubscriptionPlans) —
  // never hardcoded in the frontend, so an admin editing a plan doc updates
  // what vendors see without a redeploy.
  displayName?: string;
  features?: string[];
}
...
// Thin, presentation-shaped aliases over the raw callable responses above,
// so components consume one stable shape instead of each wiring its own
// response type directly. Fields not yet returned by any real callable are
// left out rather than guessed — see BACKEND_INTEGRATION_GUIDE-style notes
// inline where a gap exists.
...
export type VendorSubscription = EffectiveSubscription;
export type PlanDefinition = PlanOffering;
export type BillingTransaction = VendorBillingHistoryEntry;
export type VendorInvoice = InvoiceSummary;
