...
Firebase backend for the Laetiva multi-vendor marketplace, plus (as of Milestone 5) the Next.js frontend that consumes it. This repository covers five completed milestones: Auth/Users/Vendors/Verification, Catalog/Orders/Inventory/Payments/Receipts, Commerce Chat/Notifications/Blocks/Pickup Auto-Send/Support Tickets/AI Help Placeholder/Chat Moderation, Vendor Subscriptions/Plan Gating/Ratings/Invoices, and Provider-Neutral Checkout/Landing Page/CMS/Vendor Portal.
...
| **Milestone 5** — Provider-neutral checkout, subscription offerings callables, invoice/receipt renumbering, price-change policy | 32/32 acceptance tests passing in developer environment |
| **Frontend** (`web/`) — 14-page landing site, CMS editor, Vendor Portal | Built, typechecked, and production-build-verified (26/26 routes). Not yet deployed; needs real Firebase Web App config and Nneoma's legal/marketing copy. See `web/README.md`. |
| App Check | Implemented in **monitor mode only** (see App Check Rollout below). Not yet enforced. |
| Frontend integration contracts | See `docs/frontend-contracts.md` (Milestones 1-4) and `docs/LANDING_PAGE_CMS_VENDOR_PORTAL_MAPPING.md` (Milestone 5 / `web/`) |
...
| Independent security audit | Not yet performed — see the Security Posture section below and `docs/full-codebase-audit-2026-07-17.md` for an honest account of what has and has not been verified |
...
node milestone5-acceptance-tests.js
```
...
**Milestone 5** replaces per-provider checkout with a single provider-neutral flow: the frontend calls one `createSubscriptionCheckout({ plan })` callable, and the backend resolves the vendor's country (migration-safe: `businessLocation?.countryCode ?? countryCode`), selects a provider from a private country-specific priority list, and prevents double-billing on upgrade by cancelling the existing provider subscription before starting a new one. It also adds `getVendorSubscriptionOfferings`/`getPublicSubscriptionOfferings` (offerings + pricing, no provider names ever exposed to a client), renumbers invoices/receipts to `{VENDORSLUG}-{TYPE}-{seq}` (no padding, no year, immutable across slug changes), and implements the Price Change & Existing Subscriber Policy (existing subscribers keep their price until a real renewal charges the new one; 60-day notice is an operational/manual process, not backend-enforced — see `docs/LANDING_PAGE_CMS_VENDOR_PORTAL_MAPPING.md` Section 12.3). This milestone also built the `web/` frontend (landing page, CMS, Vendor Portal) and the handful of new callables it needed that didn't exist yet: `submitContactForm`, the `siteContent` CMS callables, `getVendorBillingHistory`, and `getVendorPortalAccess`.

Full request/response contracts for Milestones 1-4 are in `docs/frontend-contracts.md`; Milestone 5's contract is `docs/LANDING_PAGE_CMS_VENDOR_PORTAL_MAPPING.md`.
...
**Three payment providers are live: Paystack, Flutterwave, and Stripe.** All three converge on the exact same subscription state machine: `subscriptionWebhookCore.ts` holds the one implementation of staleness rejection, idempotency, distributed locking, out-of-order/priority resolution, and the `vendorSubscriptions` mutation itself — shared verbatim across `paystackWebhook.ts`, `flutterwaveWebhook.ts`, and `stripeWebhook.ts`. Each provider file's only job is verifying that provider's own signature scheme and mapping its payload shape into one normalized event object before handing it to the shared core.

**As of Milestone 5, provider selection is no longer a frontend concern at all.** `createFlutterwaveCheckout`/`createStripeCheckout` are no longer public callables — they became internal-only functions (`runFlutterwaveCheckout`/`runStripeCheckout`) called exclusively by the single public `createSubscriptionCheckout({ plan })`, which resolves the vendor's country server-side and picks a provider from a private, country-specific `subscriptionProviderConfig` priority list. The frontend never selects a provider and never branches on one. Adding a new provider to a country means one new webhook handler plus a `providerPlanMapping/{countryCode}-{planId}` entry — zero frontend changes, and zero changes to `vendorSubscriptions`, `resolveEffectivePlan`, `PlanLimits`, or any plan-enforcement logic.
...
Rate limiting (5 requests/60s, scoped per vendor per function) applies to every billing-sensitive vendor callable: `createSubscriptionCheckout`, `cancelSubscription`, `reactivateSubscription`, `updateInvoiceBranding`; `getPublicSubscriptionOfferings` and `submitContactForm` are rate-limited by IP instead, since they have no auth boundary at all. Admin actions (`cancelSubscriptionAdmin`, `applyManualSubscriptionOverride`) are not rate-limited but require a mandatory `reason` field and write a full before/after audit trail to `subscriptionEvents`, an immutable append-only log of every processed *and* ignored webhook event, tagged with which of the three providers sent it.
...
**What has been verified:** every Firestore collection carrying business-critical or sensitive data denies direct client writes and routes exclusively through Cloud Functions. Sensitive fields use narrow, allowlisted update rules rather than broad ownership checks — a user can mark their own notification read but cannot rewrite its title or body, for example. Authentication is delegated entirely to Firebase Auth rather than a custom credential store. Server-side business logic (block enforcement, country availability, order-state transitions) is re-validated on every relevant call rather than trusted from client state. All 390 acceptance tests across the five milestones include denial-path cases, not just success-path cases. See `docs/full-codebase-audit-2026-07-17.md` for the most recent full test run plus a codebase-wide authorization/rules/App-Check/rate-limit sweep.
...
A Next.js app covering the public landing page (14 pages), the CMS editor, and the Vendor Portal — the frontend spec'd in `docs/LANDING_PAGE_CMS_VENDOR_PORTAL_MAPPING.md`. See `web/README.md` for setup. In short: `npm install`, fill in `.env.local` with a real Firebase Web App config (`firebase apps:sdkconfig web`), `npm run dev`. Not yet deployed anywhere — this is app code only, deployment (Firebase Hosting, custom domains, SSL) is not part of this delivery.
...
- **Milestone 5 deferrals**, per `docs/LANDING_PAGE_CMS_VENDOR_PORTAL_MAPPING.md` Section 14: automated price-change notice enforcement (the 60-day notice period is an operational/manual process, not backend-enforced), automated notification workflow, full price versioning/audit history, a `pendingPriceChange` UI preview, and automated provider-side price migration tooling.
- **Frontend (`web/`) deferrals**: real legal/marketing copy (Nneoma supplies this — pages render placeholder fallback content until published through the CMS), the `www.laetiva.com`/`vendor.laetiva.com` domain split (currently one app with `/portal` and `/cms` path prefixes — a deployment-config change, not a rebuild), a one-click "restore previous version" button for legal pages (the backend already retains the prior published version), and a formal accessibility audit (acceptance-criteria-level coverage only, per scope).