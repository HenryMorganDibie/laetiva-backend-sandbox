...
- **Errors:** `permission-denied`, `resource-exhausted` (rate limited, 5/60s — each provider's checkout callable has its own independent rate-limit counter), `failed-precondition` — two distinct cases, both possible on every one of the three callables now: (a) the vendor's country has no active `subscriptionPricing` record at all — **this is a hard stop, the backend never falls back to NGN pricing or attempts currency conversion from another country's price** — or (b) pricing exists for that country but the specific provider being called isn't configured for it yet (`providerPlanMapping/{countryCode}-{planId}` exists but has no entry for that provider, or doesn't exist at all → `not-found` in that specific case).
