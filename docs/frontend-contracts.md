...
Read-only, never throws. Call this **before** rendering a "Subscribe" button so the frontend never shows a working-looking button that's actually a guaranteed checkout dead end — an active `subscriptionPricing` record with zero configured providers is deliberately treated as unavailable, not "available."
- **Auth required:** yes, role `vendor`
- **Request:** `{ plan: "standard"|"pro"|"pro_plus" }`
- **Response:** `{ success: true, available: boolean, countryCode: string, availableProviders: Array<"paystack"|"flutterwave"|"stripe">, reason?: "PRICING_NOT_CONFIGURED"|"PAYMENT_PROVIDER_NOT_CONFIGURED" }`
- **Note:** `availableProviders` tells the frontend which of the three checkout callables will actually work for this vendor right now — only offer the ones listed. `reason` is only present when `available: false`.

### `createSubscriptionCheckout` / `createFlutterwaveCheckout` / `createStripeCheckout`
Three separate callables, one per provider, all with an identical request/response shape — pick based on which provider the frontend is offering for this vendor (Paystack and Flutterwave both serve Nigeria as redundant options; Stripe is for international vendors). Call `getCheckoutAvailability` first to know which of these three will actually succeed.
- **Auth required:** yes, role `vendor`
- **Request:** `{ plan: "standard"|"pro"|"pro_plus", billingInterval?: "monthly" }` — kept on the request shape for forward compatibility with an eventual yearly rollout, but **strictly validated, never silently ignored**: omit it (defaults to monthly) or pass exactly `"monthly"`; any other value (`"yearly"` included) is rejected with `invalid-argument` — **monthly only for MVP**, no yearly checkout path exists. `plan: "basic"` is rejected (`invalid-argument`) — Basic is free, there's nothing to check out.
- **Response:** `{ success: true, authorizationUrl: string, reference: string }`
- **Errors:** `permission-denied`, `invalid-argument` (bad `plan`, `plan: "basic"`, or a `billingInterval` other than `"monthly"`/omitted), `resource-exhausted` (rate limited, 5/60s — each provider's checkout callable has its own independent rate-limit counter), `failed-precondition` — two distinct cases, both possible on every one of the three callables now, and both include a machine-readable `error.details.errorCode` so the frontend never has to parse the message string:
  - `errorCode: "PRICING_NOT_CONFIGURED"` — the vendor's country has no active `subscriptionPricing` record at all. **This is a hard stop — the backend never falls back to NGN pricing or attempts currency conversion from another country's price.**
  - `errorCode: "PAYMENT_PROVIDER_NOT_CONFIGURED"` — pricing exists for that country, but the specific provider being called isn't configured for it (`providerPlanMapping/{countryCode}-{planId}` doesn't exist, or exists but has no entry for that provider).
- **Note:** the vendor's country is read from `vendors/{vendorId}.countryCode` (set at registration via `resolveCountryCode`), not client-supplied. The returned URL is that provider's hosted checkout page. The subscription does **not** activate from this call — it activates asynchronously when that provider's webhook confirms payment. The frontend should poll `getSubscriptionStatus` or listen for a push notification after redirecting the vendor back from checkout, not assume success immediately. Once activated, `getSubscriptionStatus`'s `subscription.provider` field tells you which of the three actually processed the payment — the frontend doesn't need to remember which checkout callable it called.
...
**Public-safe plan display** — `subscriptionPlans/{planId}` is publicly readable (`allow read: if true`, no auth required) and contains only `displayName`, `features`, and the full `PlanLimits` shape — **no pricing**. Provider plan codes are in a completely separate, fully private collection (`providerPlanCodes`, legacy/plan-only) and are **never** present on this document or in any function response.

**Per-country pricing** — `subscriptionPricing/{countryCode}` is also publicly readable (`allow read: if true`) and is the only place plan prices live: `{ countryCode, currencyCode, plans: { standard: { monthlyPriceMinorUnits }, pro: {...}, pro_plus: {...} }, status }`. Amounts are integer minor currency units (e.g. `990000` for ₦9,900.00 — NGN has 2 minor-unit decimal places; this varies by currency, see `functions/src/subscriptions/countryPricing.ts` for the exponent table). `basic` is intentionally absent — it's free everywhere and never stored per-country. A frontend pricing page should read this collection directly rather than assuming a single global price. Provider-specific plan/price identifiers (`providerPlanMapping/{countryCode}-{planId}`) remain fully private, same as `providerPlanCodes`.
